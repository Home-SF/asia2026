...
(function () {
  ...
  async function uploadTrack(personKey, file, statusEl) {
    ...
    try {
      await db.collection("asia_tracks").doc(personKey).set({
  ...
  async function renderMap() {
    ...
    var tracksSnap = await db.collection("asia_tracks").get();
    ...
    var checkinsSnap = await db.collection("asia_checkins").get();
